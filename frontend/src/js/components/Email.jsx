...
export default class Email extends Component {
	constructor(props) {
		...
		this.state = {
			...
			content:
				"Consequat mauris nunc congue nisi vitae suscipit. Fringilla est ullamcorper eget nullafacilisi etiam dignissim diam. Pulvinar elementum integer enim neque volutpat actincidunt. Ornare suspendisse sed nisi lacus sed viverra tellus. Purus sit amet volutpaconsequat mauris. Elementum eu facilisis sed odio morbi. Euismod lacinia at quis risus sedvulputate odio. Morbi tincidunt ornare massa eget egestas purus viverra accumsan in. Ihendrerit gravida rutrum quisque non tellus orci ac. Pellentesque nec nam aliquam sem et.",
			pinned: false,
					...
					<ExpansionPanelSummary>
						<Box display="flex" bcackgroundColor="red" flexDirection="row" justifyContent="center" alignItems="center">
							<Container_Column order={0} paddingRight={5}>
								<ProfilePic size="small" name={this.state.sender.name ? this.state.sender.name : this.state.email} />
							</Container_Column>
							<Container_Column order={0} paddingRight={10}>
								<Container_Heading>{this.state.sender.name ? this.state.sender.name : this.state.email}</Container_Heading>
							</Container_Column>
							<Container_Column flexGrow={1} paddingRight={10}>
								<Container_SecHeading>
									<b>{this.state.subject + "...  "}</b>
									{this.state.content.substring(0, 150 - this.state.subject.length) + "..."}
								</Container_SecHeading>
							</Container_Column>
							<Container_Column alignSelf="flex-end" order={3}>
								<FormControlLabel control={<Checkbox icon={<FavoriteBorder />} checkedIcon={<Favorite />} value="checkedH" />} />
							</Container_Column>
						</Box>
					</ExpansionPanelSummary>
